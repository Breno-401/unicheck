...
- [`Unicheck/index.html`](../../Unicheck/index.html) e a porta de entrada do projeto.
- Ele nao exibe a aplicacao diretamente; funciona como pagina de redirecionamento.
- O comportamento esta ligado a [`redirect.js`](../../Unicheck/redirect.js) e [`redirect.css`](../../Unicheck/redirect.css).
...
- Google Fonts
- Imagens locais em `landing/assets/images/` e `platform/assets/images/`

### Persistencia

- Supabase Auth para sessao
- tabelas do banco para perfil, catalogo e progresso remoto dos checklists
- `js/data/checklist-data.js` como adaptador e cache local do catalogo canônico no Supabase
...
- [`supabase/diagnostics/00_inventory.sql`](../../Unicheck/supabase/diagnostics/00_inventory.sql)
- [`supabase/seeds/checklist_progress_and_seed.sql`](../../Unicheck/supabase/seeds/checklist_progress_and_seed.sql)
- [`supabase/migrations/20260824_prerelease_reset.sql`](../../Unicheck/supabase/migrations/20260824_prerelease_reset.sql)
- [`supabase/diagnostics/20260824_post_reset_validation.sql`](../../Unicheck/supabase/diagnostics/20260824_post_reset_validation.sql)
...
O comportamento dinamico principal da landing esta em [`landing/js/script.js`](../../Unicheck/landing/js/script.js), que controla:
...
- redireciona para [`platform/index-interno.html`](../../Unicheck/platform/index-interno.html) quando a sessao e valida.
...
O modulo transforma o Manual do Aluno 2024 do UniSales em uma central de orientação, com oito categorias e 53 orientações. A fonte local de 51 páginas foi auditada em 10/09/2026 e permanece em private-context/, ignorada pelo Git. A edição de 2024 não é presumida automaticamente vigente: regras são contextualizadas e dados operacionais exigem validação. A matriz por orientação está em [auditoria de conteúdo](../development/manual-auditoria-conteudo-20260910.md).

A busca é a entrada principal, acompanhada por oito atalhos de ações e categorias em linhas uniformes. O ranking mantém título, palavras-chave, resumo e corpo como sinais, incluindo requisitos, contexto e passos. Busca e categoria são preservadas no hash junto ao ID estável da orientação.

O dataset inclui campos opcionais de contexto, requisitos, passos, destinos, prazos, resultado, próxima ação e ação externa documentada. A leitura prioriza regras e procedimento, oferece navegação interna e expande apenas conteúdo complementar. Fonte, páginas e revisão existem nos dados e na documentação; não são um componente visual para o aluno. O Manual não grava progresso nem altera o Supabase. O Checklist continua responsável pela jornada e conclusão de tarefas, sem dependência técnica forte com o Manual.
...
- o conteudo editorial do Manual fica versionado em `js/data/manual-data.js`, com rastreabilidade por pagina e revisao temporal explicita; seu estado de leitura ainda nao e persistido;
...
- [`js/services/checklist.js`](../../Unicheck/js/services/checklist.js) e [`supabase/seeds/checklist_progress_and_seed.sql`](../../Unicheck/supabase/seeds/checklist_progress_and_seed.sql) agora estao alinhados em `user_checklist_item_progress`, com `user_id` e `checklist_item_id`.
...
- [`supabase/migrations/20260824_prerelease_reset.sql`](../../Unicheck/supabase/migrations/20260824_prerelease_reset.sql) remove as tres tabelas legadas, recria vazias as estruturas canônicas de usuario e centraliza RLS, grants, triggers, indices e Storage.
...
- alteracao nas regras de XP, niveis ou futuras fontes de progressao deve ser centralizada em `js/services/progression.js`;
